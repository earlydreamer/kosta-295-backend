  ...
  private Properties proFile = new Properties();
	...
	public List<Electronics> getBoardList(int pageNo) throws SQLException {
		...
		try {
			...
			int totalPage = totalCount%PageCnt.pagesize==0 ? 
					       totalCount/PageCnt.pagesize : 
						(totalCount/PageCnt.pagesize)+1;
			
			PageCnt pageCnt = new PageCnt();
			pageCnt.setPageCnt(totalPage); //4
			pageCnt.setPageNo(pageNo);//
			
			ps = con.prepareStatement(sql);
			
			ps.setInt(1, pageCnt.pagesize);     // LIMIT
			ps.setInt(2, (pageNo - 1) * pageCnt.pagesize); // OFFSET
